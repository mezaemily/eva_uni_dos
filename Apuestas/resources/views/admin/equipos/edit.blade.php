...
<div style="max-width:520px;"><div class="card">
    <form action="{{ route('admin.equipos.update', $equipo) }}" method="POST">
        ...
        <div style="margin-bottom:16px;">
            ...
            <select name="sport_id" style="width:100%;">
                @foreach($deportes as $d)
                    <option value="{{ $d->id }}" {{ old('sport_id', $equipo->sport_id) == $d->id ? 'selected' : '' }}>{{ $d->name }}</option>
                @endforeach
            </select>
        ...
        <div style="margin-bottom:24px;">
            ...
            <input type="number" name="strength" value="{{ old('strength', $equipo->strength) }}" min="1" max="100"
